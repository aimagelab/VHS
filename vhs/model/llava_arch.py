...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if self.get_vision_tower() is None or "dummy" in vision_tower:
        ...
        if getattr(self, 'mm_projector', None) is None:
            self.mm_projector = build_vision_projector(self.config, model_args)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        #print(f"image features mean {image_features.mean()} and image features variance {image_features.std()}")
        with sdp_kernel(enable_flash=False, enable_mem_efficient=False, enable_math=True):
            image_features = self.get_model().mm_projector(image_features)
        #print(f"image features mean {image_features.mean()} and image features variance {image_features.std()}")
        return image_features
    
    def encode_images_aspect_ratio(self, images):
        image_features, aspect_ratio = self.get_model().get_vision_tower()(images, return_aspect_ratio=True) # batch, 576 / 729, vision_model hidden_size
        image_features = self.get_model().mm_projector(image_features)
        return image_features, aspect_ratio

    def prepare_inputs_labels_for_multimodal(
        self, input_ids, position_ids, attention_mask, past_key_values, labels,
        images, image_sizes=None, return_image_features=False
    ):
        ...
        if type(images) is list or images.ndim == 5:
            if type(images) is list:
                if type(images[0]) is torch.Tensor:
                    ...
                    images = [[x.unsqueeze(0) if x.ndim == 3 else x for x in batch] for batch in images]
            if all(images[0].shape[1:]==image.shape[1:] for image in images):
