...
def load_models(device, model='gdino', detector_config_path=None, detector_ckpt_path=None):
    # Set this env var to point to your local mmdetection config path.
    config_path = detector_config_path or os.environ.get(
        "MMDET_CONFIG_PATH",
    ...
    OBJECT_DETECTOR = "mask2former_swin-s-p4-w7-224_8xb2-lsj-50e_coco_20220504_001756-c9d0c4f2"
    if detector_ckpt_path is not None:
        ckpt_path = os.path.abspath(detector_ckpt_path)
        if not os.path.exists(ckpt_path):
            raise FileNotFoundError(f"Segmentation detector checkpoint not found: {ckpt_path}")
    else:
        ckpt_path = os.path.join(f"{HF_HOME}/hub", f"{OBJECT_DETECTOR}.pth")
        ckpt_path_fallback = os.path.join("ckpts/geneval-det", f"{OBJECT_DETECTOR}.pth")
        if not os.path.exists(ckpt_path):
            ckpt_path = ckpt_path_fallback
            assert os.path.exists(ckpt_path)

    if not os.path.exists(config_path):
        raise FileNotFoundError(f"Segmentation detector config not found: {config_path}")
    ...
    object_names_path = os.path.join(
        os.path.dirname(__file__), "..", "inference_scripts", "object_names.txt"
    )
    with open(os.path.abspath(object_names_path)) as cls_file:
...
class SegmentationFeedback:
    ...
    def evaluate_image(self,filepath, metadata):
        ...
        # Handle new mmdet DetDataSample format
        if hasattr(result, 'pred_instances'):
            # New mmdet format with DetDataSample
            ...
            # Organize by class
            bbox = [[] for _ in range(len(self.classnames))]
            for i, (box, score, label) in enumerate(zip(bboxes, scores, labels)):
                # Append score as 5th element to match old format
                bbox_with_score = np.append(box, score)
                bbox[label].append(bbox_with_score)
            
            # Convert lists to numpy arrays
            for i in range(len(bbox)):
                if bbox[i]:
                    bbox[i] = np.array(bbox[i])
                else:
                    bbox[i] = np.empty((0, 5))
            ...
            if hasattr(pred_instances, 'masks'):
                ...
                segm = [[] for _ in range(len(self.classnames))]
                for i, (mask, label) in enumerate(zip(masks, labels)):
                    segm[label].append(mask)
...
class QwenFeedback:
    ...
    def get_randomized_feedback(self, inputs, seed=None, temperature=None):
        if seed is not None:
        ...
        if self.greedy:
            ...
            if temperature:
                generated_ids = self.model.generate(**inputs, max_new_tokens=128, temperature=temperature, do_sample=True, top_p=0.9, top_k=50)
            else:
    ...
    def evaluate_image(self,filepath, metadata):
        ...
        res = self.get_randomized_feedback(inputs)
